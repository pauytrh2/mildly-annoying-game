...
#[macroquad::main("Camera Movement with Shooting and Mouse Movement")]
async fn main() {
    ...
    loop {
        ...
        let distance = rng.random_range(0.0..300.0);

        let x = player_world_x + distance * angle.cos();
        let y = player_world_y + distance * angle.sin();
        ...
        bullets.retain(|b| {
            b.x >= world_offset_x && b.x <= world_offset_x + screen_width() &&
            b.y >= world_offset_y && b.y <= world_offset_y + screen_height()
        });
        ...
        draw_text("Move toward mouse pointer, Shoot with Left Mouse Button", 20.0, 40.0, 24.0, DARKGRAY);
        draw_text(
        ...
        if let Some(closest_enemy) = enemies.iter()
            .min_by(|a, b| {
                let da = (a.x - (player_x + world_offset_x)).hypot(a.y - (player_y + world_offset_y));
                let db = (b.x - (player_x + world_offset_x)).hypot(b.y - (player_y + world_offset_y));
...
fn lerp_angle(a: f32, b: f32, t: f32) -> f32 {
    let diff = (b - a + std::f32::consts::PI * 3.0) % (std::f32::consts::PI * 2.0) - std::f32::consts::PI;
    a + diff * t
}